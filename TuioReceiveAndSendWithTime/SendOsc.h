...
class SendOsc : public osc::OscPacketListener {
   public:
    ...
    static void SendMessageWithPositionWithTime(const char* state, int SessionID,
                                        float x, float y, timespec ts);

    static void TimeCheckOut2(double time_array2[]);
    static void TimeCheckAfter();
};
