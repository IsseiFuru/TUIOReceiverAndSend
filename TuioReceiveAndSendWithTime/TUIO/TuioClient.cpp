...
static double time_array[COUNTTIME];
struct timespec ts;
...
void TuioClient::TimeCheckOut(double time_array[]) {
    std::ofstream ofs("test.csv");
    for(int i = 0; i<= COUNTTIME; i++){
        ofs << std::setw(9) << std::setfill('0') << time_array[i] << std::endl;
    }
}

void TuioClient::TimeCheckBefore() {
    clock_gettime(CLOCK_MONOTONIC, &ts);
    // std::cout << ts.tv_nsec << std::endl;
	// std::cout << ts.tv_sec << " : " << ts.tv_nsec << std::endl;
	if(ts.tv_sec > 0){
         ts.tv_sec = 0;
         ts.tv_nsec = ts.tv_nsec + 999999999 + 1;
     }
	if(counter < 100){
         time_array[counter] = ts.tv_nsec;
         counter++;
     }else{
         TimeCheckOut(time_array);
         counter = 0;
     }
}